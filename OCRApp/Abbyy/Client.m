...
{		
	...
	NSURL* processingURL = [NSURL URLWithString:[NSString stringWithFormat:@"https://cloud.ocrsdk.com/processBusinessCard?%@", [params urlString]]];
...
- (void)processingFinished:(HTTPOperation*)operation
{
    ...
    //    NSLog(operation.error.debugDescription);
		NSParameterAssert(task);
        NSLog(@"********");
        NSLog(task.ID);
